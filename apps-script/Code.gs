...
function dispatch(p) {
  ...
  if (action === 'listSheets')
    return auth(p.token, u => ({ ok: true, sheets: listMonthlySheets(resolveId(u, p)) }));
  if (action === 'getRecords')
    return auth(p.token, u => ({ ok: true, records: getRecords(resolveId(u, p), p.sheet) }));
  if (action === 'listUsers')
    return auth(p.token, u => {
      requireAdmin(u);
  ...
  if (action === 'getCollectionHistory')
    return auth(p.token, u => {
      requireAdmin(u);
      return { ok: true, history: getCollectionHistory() };
    });

  // Authenticated write actions
  if (action === 'addRecord')
    return auth(p.token, u => addRecord(resolveId(u, p), p.sheetName, parseRecord(p.record)));
  if (action === 'updateRecord')
    return auth(p.token, u => {
      requireAdmin(u);
      return updateRecord(resolveId(u, p), p.sheetName, Number(p.rowIndex), parseRecord(p.record));
    });
  if (action === 'deleteRecord')
    return auth(p.token, u => {
      requireAdmin(u);
      return deleteRecord(resolveId(u, p), p.sheetName, Number(p.rowIndex));
    });
  if (action === 'toggleReceived')
    return auth(p.token, u => {
      requireAdmin(u);
      return toggleReceived(
        ...
        p.sheetName,
        Number(p.rowIndex),
        p.received === true || p.received === 'true'
      );
    });
...
function signup(email, password) {
  ...
  const role =
    isFirst || (ADMIN_BOOTSTRAP_EMAIL && email === ADMIN_BOOTSTRAP_EMAIL.toLowerCase())
      ? 'admin'
      : 'user';
  ...
  ensureMasterSheets().users.appendRow([
    email,
    hash,
    salt,
    role,
...
function login(email, password) {
  ...
  if (hashPassword(String(password), u.salt) !== u.passwordHash)
    return { ok: false, error: 'Incorrect password' };
  const token = createSession(email);
  return {
    ok: true,
    token,
    user: { email: u.email, role: u.role, spreadsheetId: u.spreadsheetId },
  };
}
...
function getUserFromToken(token) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === token) {
      if (Date.now() > new Date(data[i][3]).getTime()) {
        sessions.deleteRow(i + 1);
        return null;
      }
...
function migrateSheetIfNeeded(sheet) {
  ...
  if (lastCol === 8) {
    // Old 8-col format: insert Company Amount + Rider Amount after Due Amount (col 6)
    ...
    if (lastRow > 1) {
      sheet.getRange(2, 7, lastRow - 1, 1).setValue(0);
      sheet.getRange(2, 8, lastRow - 1, 1).setValue(0);
    }
  }
  if (sheet.getLastColumn() === 11) {
    // 11-col format: add Parcel Cost column (col 12)
    sheet.insertColumnsAfter(11, 1);
    sheet.getRange(1, 12).setValue('Parcel Cost');
    const hr = sheet.getRange(1, 1, 1, HEADER_ROW.length);
    hr.setFontWeight('bold');
    hr.setBackground('#1E2D45');
    hr.setFontColor('#00D4AA');
...
function getRecords(spreadsheetId, sheetName) {
  ...
  return data.slice(1).map((row, i) => ({
    ...
    transactionType: String(row[10] || 'Collection'),
    parcelCost: parseFloat(row[11]) || 0,
